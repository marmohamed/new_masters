...
class ResNetLidarBEVFusion(ResNet):
    ...
    def get_layer(self, x, indx, last_x=None, is_training=True, reuse=False):
            ...
            if indx == 0:
                ...
                last_x = conv(last_x, 192, kernel=1, stride=1, scope='conv2_' + str(i))
                last_x = batch_norm(last_x, scope='bn2_' + str(i))
                last_x = relu(last_x)
                ...
                last_x = conv(last_x, 256, kernel=1, stride=1, scope='conv3_' + str(i))
                last_x = batch_norm(last_x, scope='bn3_' + str(i))
                last_x = relu(last_x)
